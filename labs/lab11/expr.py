...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        operator_ = self.operator.eval(env)
        parameters = []
        for para in self.operands:
            parameters.append(para.eval(env))
        return operator_.apply(parameters)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        copy_parent = self.parent.copy()
        for para, arg in zip(self.parameters, arguments):
            copy_parent[para] = arg
        return self.body.eval(copy_parent)
