...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            edits, cost_ = helper_align(skeleton_idx + 1, code_idx + 1)
            possibilities.append((skel_char + edits, cost_))
        # Insert
        edits, cost_ = helper_align(skeleton_idx, code_idx + 1)
        prefix = "+[" + code_char + "]"
        possibilities.append((prefix + edits, cost_ + 1))
        # Delete
        edits, cost_ = helper_align(skeleton_idx + 1, code_idx)
        prefix = "-[" + skel_char + "]"
        possibilities.append((prefix + edits, cost_ + 1))
