    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals or vals:
            if formals is nil or vals is nil:
                raise SchemeError('Wrong number of parameters were given!')
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
    while bindings:
        first_el = bindings.first
        validate_form(first_el, 2, 2)
        names, values = Pair(first_el.first, names), Pair(scheme_eval(first_el.rest.first, env), values)
