...
def about(topic):
    ...
    def select(paragraph):
        contents = set(split(remove_punctuation(lower(paragraph))))
        for el in topic:
            if el in contents:
                return True
        return False
...
def accuracy(typed, reference):
    ...
    for t, r in zip(typed_words, reference_words):
        num_paired += (t == r)
    return 100 * num_paired / len(typed_words)
    # END PROBLEM 3
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    return closest_word if diff_function(user_word, closest_word, limit) <= limit else user_word
    # END PROBLEM 5
...
def time_per_word(times_per_player, words):
    ...
    for player in times_per_player:
        ...
        for i in range(len(player) - 1):
            time_.append(player[i + 1] - player[i])
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest_word_list = [[] for _ in players]
    for word in words:
        index = min(players, key=lambda player: time(game, player, word))
        fastest_word_list[index].append(word_at(game, word))
...
def faster_autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in memo_for_fa:
        ...
        if user_word in valid_words:
            ...
            ret = user_word if diff_function(user_word, similar_word, limit) > limit else similar_word
