...
def make_withdraw(balance, password):
    ...
    """
    "*** YOUR CODE HERE ***"
    failed_trails, state = [], True

    def withdraw(amount, input_):
        nonlocal balance, password, state
        if not state:
            return f"Too many incorrect attempts. Attempts: {failed_trails}"
        if input_ != password:
            failed_trails.append(input_)
            if len(failed_trails) == 3:
                state = False
