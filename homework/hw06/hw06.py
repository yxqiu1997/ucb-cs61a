...
class Coin:
    ...
    def worth(self):
        ...
        return self.cents + (diff - 50 if diff > 50 else 0)
...
def is_bst(t):
    ...
    def bst_min(t):
        ...
        return t.label if t.is_leaf() else min(t.label, bst_min(t.branches[0]))

    def bst_max(t):
        """Returns the maximum value of a BST"""
        return t.label if t.is_leaf() else max(t.label, bst_max(t.branches[-1]))

    if t.is_leaf():
        return True
    if len(t.branches) == 1:
        subtree = t.branches[0]
        return is_bst(subtree) and (bst_min(subtree) > t.label or bst_max(subtree) <= t.label)
    if len(t.branches) == 2:
        left_subtree, right_subtree = t.branches
        return is_bst(left_subtree) and is_bst(right_subtree) \
               and bst_max(left_subtree) <= t.label <= bst_min(right_subtree)
    return False
...
def path_yielder(t, value):
    ...
    for subtree in t.branches:
        for path in path_yielder(subtree, value):
            "*** YOUR CODE HERE ***"
            yield [t.label] + path
