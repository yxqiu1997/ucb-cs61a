...
def missing_digits(n):
    ...
    if rest == 0:
        return 0
    num_miss = 0 if last - rest % 10 <= 1 else last - rest % 10 - 1
    return missing_digits(rest) + num_miss
...
def count_change(total):
    ...
    def compute(sum_price, power):
        if sum_price == 0:
            return 1
        cur_price = 1 << power
        if sum_price < cur_price:
            return 0
        return compute(sum_price, power + 1) + compute(sum_price - cur_price, power)

    return compute(total, 0)
...
def make_anonymous_factorial():
    ...
    True
    """
    return (lambda dot: lambda num: dot(dot, num)) \
        (lambda dot, num: 1 if num == 1 else mul(num, dot(dot, sub(num, 1))))
